...
graph LR
    subgraph LangChainEventType
        ...
        OCUE["ON_CUSTOM_EVENT<br/>(claude_code_token)"]
        OCUE2["ON_CUSTOM_EVENT<br/>(claude_code_progress)"]
    end
...
classDiagram
    ...
    class claude_code {
        <<function>>
        (state: State, config: RunnableConfig) → dict
    }

    MainWorkflow ..> NodeContextResolver : uses
    MainWorkflow ..> State : operates on
    ClaudeCodeWorkflow ..> State : operates on
    ClaudeCodeWorkflow ..> claude_code : uses
...
### Claude Code ワークフロー (`ClaudeCodeWorkflow`)

Claude Code CLI を利用した単一ノードワークフロー。NodeContext を使わず、subprocess で CLI を直接呼び出す。
`adispatch_custom_event` で token / progress イベントを dispatch し、`astream_events` 経由でストリーミングする。

- **CLAUDE_CODE** — Claude Code CLI を非対話実行 (`--bare -p --output-format stream-json`)。ファイル I/O は行わずメモリ上で完結する。

```mermaid
graph LR
    START((START)) --> CC["CLAUDE_CODE<br/>Claude Code 処理中"]
    CC --> END((END))
```
...
- **`POST /test`** — HTTP API (メインワークフロー)。`stream=true` で SSE、`stream=false` で JSON レスポンス。
- **`POST /claude-code`** — HTTP API (Claude Code ワークフロー)。同上。
- **`/a2a`** — A2A プロトコル (メインワークフロー)。`LangGraphAgentExecutor` を使い、`message/send` と `message/stream` の両方に対応。
- **`/a2a-claude-code`** — A2A プロトコル (Claude Code ワークフロー)。同上。
...
classDiagram
    class FastAPI_App {
        ...
        +POST /claude-code → StreamingResponse | ChatResponse
    }
    ...
    FastAPI_App ..> LangGraphAgentExecutor : "mounted at /a2a, /a2a-claude-code"
```
